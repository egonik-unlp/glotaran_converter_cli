use clap::{arg, Parser};
use glotaran_converter_lib::{run_das6, run_lfp};
...
fn main() {
    let args = Cli::parse();
    let sync_delay = args.sync_delay.unwrap_or(das::DEFAULT_SYNC_DELAY);
    let ns_per_chn = args.ns_per_chn.unwrap_or(das::DEFAULT_NS_PER_CHN);
    let output_filename = match args
        .output_filename
        .unwrap_or(das::DEFAULT_OUTPUT_FILENAME.into())
    {
